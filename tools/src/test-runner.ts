...
class TestRunner {
  ...
  private testDir = path.join(__dirname, 'tmp_rovodev_test');
  private testFile = path.join(this.testDir, 'test-file.txt');
  private testContent = 'This is a test file for filesystem viewer utility';
  ...
  private async testGetLastModificationDate() {
    console.log('Testing get_last_modification_date_of_file...');
    
    try {
      const modDate = await get_last_modification_date_of_file(this.testFile);
  ...
  private async testGetLastModificationDateSync() {
    console.log('Testing get_last_modification_date_of_file_sync...');
    
    try {
      const modDate = get_last_modification_date_of_file_sync(this.testFile);
      this.assert(modDate instanceof Date, 'Should return a Date object');
      this.assert(modDate.getTime() > 0, 'Should return a valid timestamp');
  ...
  private async testIsFile() {
    console.log('Testing isFile...');
    
    try {
      const fileResult = await isFile(this.testFile);
      const dirResult = await isFile(this.testDir);
      const nonExistentResult = await isFile(path.join(this.testDir, 'non-existent.txt'));
  ...
  private async testIsDirectory() {
    console.log('Testing isDirectory...');
    
    try {
      const dirResult = await isDirectory(this.testDir);
      const fileResult = await isDirectory(this.testFile);
      const nonExistentResult = await isDirectory(path.join(this.testDir, 'non-existent-dir'));
  ...
  private async testGetFileInfo() {
    console.log('Testing getFileInfo...');
    
    try {
      const fileInfo = await getFileInfo(this.testFile);
  ...
  private async testErrorHandling() {
    ...
    try {
      ...
      // Test async version
      try {
        await get_last_modification_date_of_file(nonExistentFile);
        this.assert(false, 'Should throw error for non-existent file');
      } catch (error) {
        this.assert(error instanceof Error, 'Should throw Error object');
        this.assert((error as Error).message.includes('Failed to get modification date'), 'Should have descriptive error message');
      }
      
      // Test sync version
      try {
        get_last_modification_date_of_file_sync(nonExistentFile);
        ...
        this.assert((error as Error).message.includes('Failed to get modification date'), 'Should have descriptive error message (sync)');
      }
